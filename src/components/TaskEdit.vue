...
const handleSubmit = async () => {
  ...
  const dueByDate = form.dueByDate
...
async function handleChange(event) {
  ...
  const dueByDate = form.dueByDate?.substring(0, 10)
    ...
    <form v-if="state.isLoaded" @submit.prevent="handleSubmit()">
      ...
      <div id="list-selection" class="flex gap-2 mb-4 items-stretch">
        ...
        <select v-if="!form.isDone" v-model="form.listId" id="listId" name="listId" @change="handleChange"
          ...
          <option v-for="list in backendStore.lists.filter(list => list.id !== backendStore.doneList.id)" :key="list.id"
            :value="list.id">{{
              list.name }}
          </option>
        </select>
      ...
      <div id="title-field" class="mb-4">
        <input v-model="form.title" type="text" id="title" name="title" placeholder="Task title" @change="handleChange"
          @keydown.capture="handleCmdEnterInField" class="border rounded w-full p-2 mb-2" required
          v-bind:autofocus="state.isNew" />
      </div>
      ...
      <div id="parent-task-field" class="mb-4 flex gap-2 items-baseline" ref="parentTaskRef">
        <div v-if="form.parentTaskId" class=" flex justify-between flex-nowrap gap-2 items-baseline">
          ...
          <button type="button" title="Remove link to parent"
            class="cursor-pointer rounded-4xl  hover:bg-orange-50 w-6 h-6" @click.prevent=" handleRemoveLinkToParent()">
      ...
      <div id="child-tasks-field" class="mb-4" ref="childTasksRef">
        ...
        <ul id="childTasks" v-if="state.childTasks.length" class="mb-2">
          <li v-for="childTask in state.childTasks" :key="childTask.id"
            ...
            <span>
              <span v-if="childTask.isDone">
                <i class="pi pi-check-circle mr-2 bg-green-600 rounded-4xl"></i>
              </span>
              <span v-else>
